...
class WaveHelper:
    ...
    def _make_numpy_array_from_energies(self):
        for harm in self._harmonic_to_energy_dict.keys():
            ...
            if (
                np.arange(start, stop + step, step) == np.array(self._harmonic_to_energy_dict[harm])
            ).all:
    ...
    def _make_energy_to_file_dict(self):
        for harm in self._harmonic_to_folders_dict.keys():
            ...
            self.energies_to_file_dict[harm] = dict(zip(energies_keys, energy_files, strict=False))
    ...
    def report_available_energies(self, verbose=True):
        ...
        Args:
            verbose (bool, optional): If Ture a report about the neergie is printed.
                                        Defaults to True.
        """
        ...
        if verbose:
            ...
            if self._harmonic_to_energies_array is not False:
                ...
                for harm in self._harmonic_to_energy_dict.keys():
                    ...
                    print(
                        f"step \
                        {self._harmonic_to_energy_dict[harm][1]-self._harmonic_to_energy_dict[harm][0]}"
                    )
            else:
                for harm in self._harmonic_to_energy_dict.keys():
                    print(f"Harmonic number {harm}, available energies:")
