"""Eval for simulation_multilayer_monochromator_efficiency.py.

Photon energy vs bandwidth and flux (ph/s when source flux is known, else %),
one curve per cFactor.
"""
...
if __name__ == "__main__":
    ...
    csv_path = os.path.join(this_file_dir, "RAYPy_Simulation_ML_Mono",
                            "DetectorAtFocus_RawRaysOutgoing.csv")
    group_col = "PG.cFactor"
    title = "ML monochromator efficiency: bandwidth & flux vs energy"
    ...
    if group_col:
        ...
        axs[1].legend()
    fig.tight_layout()
    out_png = os.path.join(this_file_dir, "eval_multilayer_monochromator_efficiency.png")
