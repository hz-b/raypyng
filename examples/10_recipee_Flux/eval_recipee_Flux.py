"""Eval for simulation_recipee_Flux.py (Flux recipe, RAY-UI analysis).

The Flux recipe turns all reflectivities on and uses RAY-UI analysis
(ScalarBeamProperties). There is no combined CSV, so we read looper.csv (energy
per simulation) and the per-simulation
round_0/<N>_DetectorAtFocus-ScalarBeamProperties.csv files, then plot photon
energy vs bandwidth (energyH_width) and vs transmission efficiency (%).
"""
...
if __name__ == "__main__":
    ...
    sim_dir = os.path.join(this_file_dir, "RAYPy_Simulation_Flux")
    oe = "DetectorAtFocus"
    title = "Flux recipe: bandwidth & efficiency vs energy"
    out_png = os.path.join(this_file_dir, "eval_recipee_Flux.png")
