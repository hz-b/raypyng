"""Eval for simulation_recipee_ResolvinPower.py (ResolvingPower recipe).

Uses RAY-UI analysis (ScalarBeamProperties). There is no combined CSV, so we
read looper.csv (energy per simulation) and the per-simulation
round_0/<N>_DetectorAtFocus-ScalarBeamProperties.csv files, then plot photon
energy vs resolving power E/ΔE (from energyH_width) and vs efficiency (%).
"""
...
if __name__ == "__main__":
    ...
    sim_dir = os.path.join(this_file_dir, "RAYPy_Simulation_RP")
    oe = "DetectorAtFocus"
    title = "ResolvingPower recipe: E/ΔE & efficiency vs energy"
    out_png = os.path.join(this_file_dir, "eval_recipee_ResolvinPower.png")
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        resolving_power = df["PhotonEnergy"] / df["energyH_width"]
    axs[0].plot(df["PhotonEnergy"], resolving_power, marker=".")
    axs[0].set(xlabel="Photon energy [eV]", ylabel="Resolving power E/ΔE", title=title)
